...
class Master(MasterBaseType):
    ...
    def shortDownload(self, address, addressExt, data):
        ...
        response = self.transport.request(types.Command.SHORT_DOWNLOAD, length, 0, addressExt, *addr_data)
        return response
    ...
    def setDaqListMode(self, mode, daqListNumber, eventChannelNumber, prescaler, priority):
        ...
        response = self.transport.request(
            types.Command.SET_DAQ_LIST_MODE, mode, *flatten(dln, ecn, [prescaler, priority])
        )
        return response
