...
ASAM_XCP_ON_BT_GUID = "dc3ccedc-f7de-56e8-876f-781c073c9b77"

def confirm_service(address):
    matches = bluetooth.find_service(uuid = ASAM_XCP_ON_BT_GUID, address = address)
    if len(matches) > 0:
        match = matches[0]
        return (match["name"], match["description"], match["host"], match["port"])
    else:
        return None

#print(confirm_service("00:15:83:E8:51:AE"))

class Bth(BaseTransport):
    """"""
