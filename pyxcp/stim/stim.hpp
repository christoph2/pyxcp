...
struct DaqEventInfo {
    ...
    explicit DaqEventInfo(/*std::string_view*/const std::string& name, std::int8_t unit_exp, std::size_t cycle,
...
public:
    ...
    void setDaqListMode(std::uint16_t mode,std::uint16_t daqListNumber, std::uint16_t eventChannelNumber,
                        std::uint16_t prescaler, std::uint16_t priority) {
        if (!validateEntryNumber(daqListNumber)) {
            return;
        }
        auto entry = m_daq_lists[daqListNumber];
...
protected:
    ...
    bool validateEntryNumber(std::uint16_t daqListNumber, std::optional<std::uint16_t> odtNumber=std::nullopt,
                        std::optional<std::uint16_t> odtEntryNumber=std::nullopt) const {
