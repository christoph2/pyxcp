...
class PyXcpLogFileUnfolder : public XcpLogFileUnfolder {
   public:
    ...
    void on_daq_list(std::uint16_t daq_list_num, double timestamp0, double timestamp1, const std::vector<measurement_value_t>& measurement) override {
...
PYBIND11_MODULE(rekorder, m) {
    ...
    py::class_<MeasurementParameters>(m, "MeasurementParameters")
        .def(py::init<
             std::uint8_t, std::uint8_t, bool, bool, bool, bool, double, std::uint8_t, std::uint16_t, const std::vector<DaqList>&, const std::vector<std::uint16_t>&>(
        ))
        .def("dumps", [](const MeasurementParameters& self) {
            return py::bytes(self.dumps());
        })
		.def("__repr__",[](const MeasurementParameters& self) {
			std::stringstream ss;
